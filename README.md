...
| `/` | 1.73s | 0.000 |
| `/works/` | 1.73s | 0.019 |
| `/about-this-site/` | 2.25s | 0.000 |
| `/works/lumen-editor/` | 1.95s | 0.000 |

> **`/works/` だけ CLS が 0 ではありません。** 看板の和文がフォールバックで描かれたあと
> 本来の書体に差し替わり、行の高さが変わるためです。予算 0.05 の範囲内なので許容しています。
> 和文 Regular の preload は**採用しませんでした**。CLS は 0.019 のまま変わらず、
